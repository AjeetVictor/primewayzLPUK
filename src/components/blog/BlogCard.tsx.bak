...
export const BlogCard = ({ post, featured = false }: BlogCardProps) => (
  <article
    className={`group h-full overflow-hidden border border-zinc-100 bg-white shadow-sm shadow-zinc-900/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-900/10 ${
      featured ? 'rounded-[2rem] lg:grid lg:grid-cols-[1.05fr_0.95fr]' : 'rounded-[1.75rem] flex flex-col'
    }`}
  >
    <Link to={`/blog/${post.id}`} className={featured ? 'contents' : 'flex h-full flex-col'}>
      {post.image && (
        <div className={`relative overflow-hidden ${featured ? 'min-h-[280px] lg:min-h-full' : 'aspect-[16/10]'}`}>
          <img
            src={post.image}
            alt={post.title}
      ...
      <div className={`flex flex-1 flex-col ${featured ? 'p-8 sm:p-10 lg:p-12' : 'p-7'}`}>
        ...
        <h3
          className={`font-bold tracking-tight text-zinc-900 transition-colors group-hover:text-emerald-600 ${
