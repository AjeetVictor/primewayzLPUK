...
function optimizeHeadAssetOrder(html: string): string {
  ...
  const firstStylesheetHref = firstStylesheet?.match(/href=["']([^"']+)["']/i)?.[1];

  if (firstStylesheet && firstStylesheetHref && !optimizedHead.includes(`rel="preload" as="style" href="${firstStylesheetHref}"`)) {
    const preloadTag = `<link rel="preload" as="style" href="${firstStylesheetHref}" />`;
    optimizedHead = optimizedHead.replace(
      firstStylesheet.trim(),
      `${preloadTag}\n  ${firstStylesheet.trim()}`
    );
  }
...
async function startServer() {
  if (process.env.SKIP_ADMIN_SEED === 'true') {
    ...
    try {
      ...
      backendLog('admin seed failed; server will continue without database initialization', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
...
export { sendDailyLeadSummaryEmail };
